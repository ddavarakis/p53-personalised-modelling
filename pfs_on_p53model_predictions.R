...
pfs_surv_obj <- Surv(time = all_predictions$PFS_MONTHS, 
                     event = all_predictions$PFS_STATUS)
...
p53_cox_univariate <- function(p53_state, range) {
  ...
  vec
  vec <- append(vec,c("PFS_STATUS", "PFS_MONTHS"),after=0)
  print(vec)
  data <- all_predictions[,vec]
  tbl_uvregression(
    data,
    method=coxph,
    y = Surv(time = PFS_MONTHS, event = PFS_STATUS),
    exponentiate = TRUE,
    #include = -ID
  )
}
...
# Statistical significant p54 states are:
p53_sign_states <- c("s15_7")
vec_s46 <- paste0("s46_", 8:9)
p53_sign_states <- append(p53_sign_states, vec_s46)
p53_sign_states
vec_s46 <- paste0("s46_", 11:30)
p53_sign_states <- append(p53_sign_states, vec_s46)
p53_sign_states
...
# Multivariant Cox Regression for each p53_significant_state + "ARF1", "ARL4C", "MDM4"
for (p53_state in p53_sign_states) {
  print(p53_state)
  fS <- pfs_surv_obj ~ .
  genes <- c("ARF1", "ARL4C", "MDM4")
...
write.xlsx(df,"pfs_significant_genes.xlsx",sheetName = "pfs_citated_genes")

# ---------------------------------
# Multivariant Cox regression analysis 
# for: significant_p53_state + ARF1 + ARL4C + MDM4 + stage2 + stage3 + stage4
...
for (p53_state in p53_sign_states) {
  print(p53_state)
  fS <- pfs_surv_obj ~ .
  others <- c("ARF1", "ARL4C", "MDM4", "stage2", "stage3", "stage4")
...
my_x <- as.matrix(my_x)
# get the  time
my_time <- all_predictions$PFS_MONTHS
# get the status
my_status <- all_predictions$PFS_STATUS
...
# λ = 0.077290 returns the 21 most significant genes (given to Cillian)
# λ = 0.067230 returns the 32 most significant genes
my_cfs = coef(my_fit, s = 0.077290) # gets 21 genes

# get the relevant genes
my_meaning_coefs = rownames(my_cfs)[my_cfs[,1]!= 0]
my_meaning_coefs
df <- data.frame(my_meaning_coefs)
write.xlsx(df,"pfs_significant_genes.xlsx",sheetName = "pfs_significant_genes",append = TRUE,row.names = FALSE)
...
df <- data.frame(my_meaning_coefs, stat_genes)
write.xlsx(df,"pfs_significant_genes.xlsx",sheetName = "pfs_significant_genes",append = TRUE,row.names = FALSE)
...
for (p53state_name in p53_sign_states) {
  ...
  for (i in 2:(length(p53state_sorted)-1)) {
    ...
    if (cur_cutoff>0) {
      ...
      form <- paste0("pfs_surv_obj ~ ",p53state_strata)
